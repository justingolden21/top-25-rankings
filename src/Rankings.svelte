<script>
	...
	onMount(() => {
		...
		let found = false;
		const slugOne = params.slugOne.split('-').join(' ').toLowerCase();
		const slugTwo = params.slugTwo ? params.slugTwo.split('-').join(' ').toLowerCase() : '';
		console.log(slugOne);
		if (
			['mens college basketball', 'womens college basketball', 'college football'].includes(slugOne)
		) {
			sport = slugOne;
			found = true;
		}
		if (['ap', 'coaches', 'fcs coaches', 'div ii coaches'].includes(slugTwo)) {
			if (
				slugTwo !== 'fcs coaches' ||
				slugTwo !== 'div ii coaches' ||
...
<select
	...
	on:change={() => {
		...
		goto(`/${sport.split(' ').join('-')}/${pollType.split(' ').join('-')}`);
		getData();
	}}
